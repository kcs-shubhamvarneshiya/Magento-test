...
class Uploader extends \Magento\MediaStorage\Model\File\Uploader
{
    ...
    public function move($fileName, $renameFileOff = false)
    {
        ...
        if (preg_match('/\bhttps?:\/\//i', $fileName, $matches)) {
            ...
            $tmpFilePathAbs = $this->_directory->getAbsolutePath($tmpFilePath);
            try {
                $image = new Imagick($tmpFilePathAbs);
                $image->profileImage('*', null);
                $image->setInterlaceScheme(Imagick::INTERLACE_NO);

                $image->writeImage($tmpFilePathAbs);
            } catch (\Exception $exception) {
                $this->logger->error(
                    '\Capgemini\ImportExport\Model\Import\Uploader: ' . $exception->getMessage(),
                    ['image' => $tmpFilePath]
                );
            }
        }
    ...
    protected function _setUploadFile($filePath)
    {
        ...
        if (!$readable) {
            $this->logger->log(LogLevel::NOTICE, $filePath);
        }
