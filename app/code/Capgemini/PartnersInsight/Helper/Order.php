...
use Capgemini\PartnersInsight\Model\Api\Order as PartnersInsightorder;
...
class Order extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    /**
     * @var PartnersInsightorder
     */
     protected $partnersInsightorder;
    ...
    public function __construct(
        ...
        PartnersInsightorder $partnersInsightorder,
        ...
        $this->logger = $logger;
        $this->partnersInsightorder = $partnersInsightorder;
        $this->jsonSerializer = $jsonSerializer;
    }
     public function getOrderDataApi($orderId){
        return $this->partnersInsightorder->getOrder($orderId);
    }
    ...
    protected function _convertOrderItem(array $orderItemData)
    {
        ...
        $data['qty_ordered'] = $orderItemData['shipping']['quantityOrdered'];
        $data['qty_shipped'] = $orderItemData['shipping']['quantityShipped'];
        $data['tracking'] = $orderItemData['shipping']['carrier']['trackingNumber'];
        $data['tracking_url'] = $orderItemData['shipping']['carrier']['trackingUrl'] ?? '';
        $data['carrier_name'] = $orderItemData['shipping']['carrier']['name'] ?? '';
        $qty = $data['qty_ordered'] ?? null;
        $qtyCancelled = $data['qty_canceled'] ?? null;
    ...
    protected function _convertOrder(array $orderData)
    {
        ...
        $this->logger->warning("PI ORDER DATA ". $this->jsonSerializer->serialize($orderData));

        $data = [];
        $incrementId = $orderData['orderId'];
        $realIncrementId = $orderData['orderId'];
        ...
        $order->setCreatedAt($this->convertDate($orderData['orderDate']));
        $this->logger->warning("PI ORDER MID ". $this->jsonSerializer->serialize($order));
        // $realOrder is used for reordering

        $source = trim(strtolower($orderData['orderStatus']));
        switch ($source) {
            case 'open':
    ...
    public function partnersInsightOrderToMagentoOrder(array $orderData)
    {
        ...
        $order = $this->_convertOrder($orderData['data']);
...
//        }
        ...
        $items = [];
        $itemsData = $orderData['data']['orderLines'] ?: [];
        ...
        $this->logger->warning("PI ORDER ITEMS ". $this->jsonSerializer->serialize($items));
        ...
        $shippingAddress = $this->_convertAddress($orderData['data']['deliverToAddress'], $firstName, $lastName);

        $order->setBillingAddress($billingAddress);
        $order->setShippingAddress($shippingAddress);

        $this->logger->warning("PI ORDER AFTER ". $this->jsonSerializer->serialize($order));
