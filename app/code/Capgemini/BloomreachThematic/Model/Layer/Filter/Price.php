...
namespace Capgemini\BloomreachThematic\Model\Layer\Filter;

use Amasty\Shopby\Model\Layer\Filter\Price as Orig;
...
class Price extends Orig
{
    ...
    public function __construct(
        ...
        parent::__construct(
            ...
            $filterSettingResolver,
            $decimalFilterSettingResolver,
            $decimalFilterRequestDataResolver,
            $filterConfigResolver,
            $getPrecisionValue,
            $request,
            $removeExtraZeros, $data
        );
    ...
    private function getSearchResult(): ?SearchResultInterface
    {
        ...
        if ($this->filterRequestDataResolver->hasCurrentValue($this)) {
            $requestParams = $this->moduleHelper->getRequestParams();
            $paramName = $this->getRequestVar();
            unset($requestParams[$paramName]);
            $alteredQueryResponse = $this->thematicSearch->search($requestParams);
        }
