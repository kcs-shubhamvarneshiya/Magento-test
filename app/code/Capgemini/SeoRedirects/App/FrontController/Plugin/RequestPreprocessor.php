 ...
 */
namespace Capgemini\SeoRedirects\App\FrontController\Plugin;

/**
 * Overrides Magento\Store\App\FrontController\Plugin\RequestPreprocessor to prevent stripping query parameters
 * when redirecting to base URL
 */
            ...
            if ($baseUrl) {
                ...
                if (!$this->getBaseUrlChecker()->execute($uri, $request)) {
                    $redirectUrl = $this->_url->getRedirectUrl(
                        $this->_url->getDirectUrl(ltrim($request->getRequestUri(), '/'), ['_nosid' => true])
                    );
