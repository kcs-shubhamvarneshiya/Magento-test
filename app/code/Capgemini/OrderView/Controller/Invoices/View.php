...
use Capgemini\PartnersInsight\Model\Api\Invoice as PiOrderApi;
use Capgemini\PartnersInsight\Helper\Invoice  as PiOrderHelper;
...
class View extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $order = $this->_getInvoice($orderNumber);
        ...
        $resultPage->getConfig()->getTitle()->set(__('Invoice #%1', $orderNumber));
    ...
    protected function _getInvoice($orderId)
    {
        if ($this->order === null) {
            // check for wholesale customers
            if ($this->helper->getCustomerType() == Config::WHOLESALE) {
                $orderData = $this->piOrderApi->getInvoice($orderId);
