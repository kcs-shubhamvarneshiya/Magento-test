...
class GAClient {
    ...
    const GOOGLE_ANALYTICS_SERVERSIDE_USE_QUEUE      = 'google/serverside_analytics/use_queue';
    ...
    /**
     * Capgemini_ServerSideAnalytics_Model_GAClient constructor.
    ...
    public function firePurchaseEvent()
    {
        ...
        if ($this->scopeConfig->isSetFlag(self::GOOGLE_ANALYTICS_SERVERSIDE_DEBUG_MODE)) {
            $this->logger->info('capgemini_serversideanalytics_debug_response: ', array($response->getDebugResponse()));
        }
        if ($this->scopeConfig->isSetFlag(self::GOOGLE_ANALYTICS_SERVERSIDE_ENABLE_LOGGING)) {
            $this->logger->info('capgemini_serversideanalytics_requests: ', array($response->getRequestUrl()));
        }
