    ...
    $.widget('mage.amScrollScript', {
        ...
        _preloadPageAfter: function (page) {
            ...
            if (nextPage && nextPage <= this.pagesCount) {
                ...
                $(document).trigger('amScroll:updateProductList');
            }
        },
        ...
        {
            if (this.productsCounter) {
                this.productsCounter.remove();
            }
            var showedProducts = $('.product-items .product-item').length;
            var allProducts = $(this.totalAmountSelector).first().find(this.totalNumberSelector).first().text();
            this.productsCounter = $('<div class="amscroll-products-counter">Showing <span class="showed-products">' + showedProducts + '</span> of <span class="all-products">' + allProducts + '</span> products.</div>');
            this._insertBlockInTheEnd(this.productsCounter);
        },

        _updateCounter: function()
        {
            var showedProducts = $('.product-items .product-item').length;
            this.productsCounter.find('.showed-products').html(showedProducts);
        },
        ...
        _insertBlockInTheBegin: function (block) {
            var element = $('.' + this.classes.pageNumber + ', ' + this.options['product_container']).first(),
                ...
                lastButton = lastPage.nextAll(loadButtonSelector),
                lastLoadingBLock = lastPage.nextAll(loadingBlockSelector);
